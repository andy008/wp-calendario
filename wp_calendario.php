...
//	$output .=						'<span id="custom-current" class="custom-current" title="Go to current date"></span>';
	...
	$output .='<script>
				...
				$( "#custom-next" ).on( "click", function() {
					if(can_do("next")) cal.gotoNextMonth( updateMonthYear );
				} );
				$( "#custom-prev" ).on( "click", function() {
					if(can_do("prev")) cal.gotoPreviousMonth( updateMonthYear );
				} );
				...
				function can_do(to_do){
					var min_month='. $theinitMonth .';
					var max_month='. $rangeMaxmonth .';
					var year='. $rangeYear . ';
					if(to_do=="next"){
